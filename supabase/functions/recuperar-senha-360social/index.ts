...
Deno.serve(async req=>{
  ...
  try{
    ...
    const {data:equipe}=await admin.schema('social').from('equipe').select('nome,auth_id').ilike('email',email).eq('ativo',true).eq('tem_acesso',true).limit(1).maybeSingle();
    if(!equipe?.auth_id)return json(publica);
    ...
    const nome=esc(String(equipe.nome||'').trim().split(/\s+/)[0]||'Olá');
    ...
    const envio=await fetch('https://api.brevo.com/v3/smtp/email',{method:'POST',headers:{'api-key':Deno.env.get('BREVO_API_KEY')!,'content-type':'application/json'},body:JSON.stringify({sender:{name:'360social · Arbor Labs',email:'contato@arborlabs.com.br'},replyTo:{name:'Arbor Labs',email:'contato@arborlabs.com.br'},to:[{email,name:equipe.nome}],subject:'RECUPERAÇÃO DE SENHA - 360SOCIAL',htmlContent:html,tags:['360social','recuperacao']})});
